...
class Email extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      mailsContent: (
        <tr>
          <td colSpan="2" id="noselected_div">
            No Folder selected.
          </td>
        </tr>
      ),
      ...
      mailBody: <div className="noselected_div">No Mail selected.</div>,
      ...
      utterText:
        " To Send Email, please say Send Email. To Listen Email, say Listen. and To Exit, say Logout",
      initial: true,
      sendEmail: false,
      inboxEmail: false,
      sentEmail: false,
        ...
        >
          <div className="form-group">
            <div className="col-3 col-sm-12">
              <label className="form-label" htmlFor="input-example-4">
                <h5>To: </h5>
              </label>
            </div>
          ...
          <div className="form-group">
            <div className="col-3 col-sm-12">
              <label className="form-label" htmlFor="input-example-5">
                <h5>Subject: </h5>
              </label>
            </div>
            <div className="col-9 col-sm-12">
          ...
          <div className="form-group">
            <div className="col-3 col-sm-12">
              <label className="form-label" htmlFor="input-example-6">
                <h5>Message: </h5>
              </label>
            </div>
            <div className="col-9 col-sm-12">
    ...
    Axios.post("/email/send_email", {
      ...
      if (req.data.code === SUCCESS) {
        ...
        try {
          ...
          console.log("Could not clear fields");
        }
        ...
        // alert(req.data.detail) // You can keep or remove alerts
      } else {
        alert(req.data.detail);
      }
    });
  ...
  handleEnd(err, text) {
    ...
    if (command === "menu" || command === "main menu") {
      ...
      // Clear temporary data arrays
      sendingInfo = [];
      allText = [];

      this.text2speech(
        "Main Menu. You can say Send Email, Listen Email, or Logout.",
      );
      return;
    }
    ...
    if (this.state.inboxEmail === true) {
      if (command === "restart") {
        ...
        this.text2speech(
          speech + "! . ! Say the index number to open an email.",
        );
      } else if (!isNaN(command)) {
        // If user says a number (Index)
        var mail = this.state.InboxMails[parseInt(command) - 1];
        if (mail) {
        ...
        this.text2speech(
          "I didn't understand. Say an email number, or say Menu.",
        );
      }
    }
    ...
    // --- SENT MODE ---
      ...
      if (command === "restart") {
        ...
        var mail = this.state.SentMails[parseInt(command) - 1];
        if (mail) {
          this.mailContent(mail, 0);
          this.text2speech(
            "To: " +
    ...
    // --- SENDING EMAIL MODE ---
      ...
      sendingInfo.push(text); // Add what was spoken to our list

      // Check if user said "Send"
      if (sendingInfo[sendingInfo.length - 1].toLowerCase() === "send") {
        // CLEAN EMAIL ADDRESS (Remove spaces, fix 'at the rate')
        ...
        this.text2speech(
          "I have: To " +
            ...
            ", Subject " +
            this.state.subject_to_send +
            ". Say Correct to send, or Restart to try again.",
        );
      }

      if (sendingInfo[sendingInfo.length - 1].toLowerCase() === "menu") {
        sendingInfo = [];
        this.setState({ sendEmail: false, text: "" });
        this.text2speech(
          "To Send Email, please say Send Email. To Listen Email, say Listen. and To Exit, say Logout",
        );
        ...
        sendingInfo = [];
        this.text2speech(
          "Okay, starting over. Say Address, then Subject, then Message.",
        );
    ...
    // ==============================
      ...
      if (command.includes("listen")) {
        this.text2speech(
          "To hear received mails, say Inbox. To hear sent mails, say Sent.",
        );
      } else if (
        ...
        this.sendMail();
        this.text2speech(
          "Say the Address, then Subject, then Message. Then say Send.",
        );
        this.setState({ sendEmail: true, sentEmail: false, inboxEmail: false });
        sendingInfo = []; // Clear previous data
      } else if (
        allText[allText.length - 1].toLowerCase().replace(/ /g, "") ===
        "sendemail"
      ) {
        this.sendMail();
        this.text2speech(
          `Please say the address, subject, and message. Then say send.`,
        );

        // --- FIX: ENSURE PREVIOUS DATA IS GONE ---
        sendingInfo = [];
        // -----------------------------------------
        ...
        this.text2speech(
          "I didn't catch that. Say Send Email, Listen, or Logout.",
        );
      }
    }
  }
  render() {
    // REMOVED INITIAL CHECK FROM RENDER TO AVOID LOOP
        ...
        <div className="app_div">
          ...
          <div className="menu_div">
            <ul className="menu">
              ...
              <li className="menu-item" onClick={this.sendMail}>
                <a href=" #top">Send Email</a>
              </li>
              <li className="menu-item">
                <a href="#top">Listen Email</a>
              </li>
              <li className="menu-item">
                <a href="#top" onClick={this.handleLogout}>
                  Logout
                </a>
              </li>
            </ul>
            ...
            <div className="mailbox_div">
              ...
              <ul className="mailboxitem_div">
                <li className="item_div" key={0}>
                  <button
                    className="btn badge"
                    data-badge={this.state.InboxMails.length}
                    onClick={this.inboxFunction}
                  >
                    Inbox
                  </button>
                </li>
                ...
                <li className="item_div" key={1}>
                  <button
                    className="btn badge"
                    data-badge={this.state.SentMails.length}
                    onClick={this.sentFunction}
                  >
                    Sent
                  </button>
                </li>
          ...
          <div className="mails_div">
            <table className="email-list table table-striped table-condensed">
              ...
              <tbody>{this.state.mailsContent}</tbody>
            </table>
          </div>

          <div className="mailcontent_div">{this.state.mailBody}</div>
        </div>
      </div>
    );
  }
