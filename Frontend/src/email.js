    ...
    handleSendSubmit(e) {
        ...
        Axios.post("/email/send_email", {
            ...
            if (req.data.code === SUCCESS) {
                ...
                // --- FIX: UNCOMMENT ALERT (So you see the success message) ---
                alert(req.data.detail); 

                // --- FIX: MANUALLY CLEAR INPUT FIELDS (So the text disappears) ---
                try {
            ...
            // Log the error so we can see why it failed
   ...
   //This function ends the speech to text process and speech will be saved
    handleEnd(err, text) {
        ...
        if (this.state.inboxEmail === true) {
            if (command === "restart") {
                // FIX: Changed var to let to avoid redeclare error
        ...
        // ==============================
            ...
            else if (allText[allText.length - 1] && allText[allText.length - 1].toLowerCase().replace(/ /g, "") === "sendemail") {
                this.sendMail();
